...
try {
    ...
    if ($currentUser['role'] === 'employee' || ($currentUser['role'] === 'student' && ($currentUser['position'] === 'SSC President' || $currentUser['position'] === 'CSC President'))) {
        ...
        $stmt = $db->prepare("
            SELECT d.id, d.title, 'document_status' as type, NOW() as timestamp, al.details
            FROM documents d
            JOIN audit_logs al ON al.target_id = d.id AND al.target_type = 'Document'
            WHERE d.student_id = ? AND al.action IN ('DOCUMENT_APPROVED', 'DOCUMENT_REJECTED', 'DOCUMENT_SIGNED')
            ORDER BY al.id DESC
        ");
        $stmt->execute([$currentUser['id']]);
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
