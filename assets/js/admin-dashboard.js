...
window.changeDocsLimit = function() {
    ...
    if (limitDropdown) {
        ...
        window.loadDocuments(1, currentDocsStatus, currentDocsSearch, currentDocsDept, currentDocsType);
    }
};
...
window.loadDocuments = async function(page = 1, status = undefined, search = undefined, dept = undefined, type = undefined) {
    ...
    if (type !== undefined) currentDocsType = type;
    currentDocsPage = page;
    try {
        const p = newSearchParams({ type: 'documents', page, limit: docsPageSize, status: currentDocsStatus, search: currentDocsSearch, department: currentDocsDept, doc_type: currentDocsType });
        const d = await apiFetch(`${ADMIN_DATA_API}?${p}`);
        if (d.success) {
            publicDocuments = (d.documents || []).filter(doc => {
                const okStatus = typeof matchesStatusFilter === 'function'
                    ? matchesStatusFilter(doc.status, currentDocsStatus || 'all', 'document')
                    : true;
                const okDept = typeof matchesDepartmentFilter === 'function'
                    ? matchesDepartmentFilter(doc.department || '', currentDocsDept || '')
                    : true;
...
function createDocumentRow(doc) {
    ...
    row.innerHTML = `
        ...
        <td><span class="badge ${statusClass} rounded-pill px-3">${doc.status.toUpperCase()}</span></td>
        <td class="text-sm text-muted">${new Date(doc.uploaded_at).toLocaleDateString()}</td>
        <td class="text-end pe-4">
            <button class="btn btn-ghost btn-sm text-primary rounded-circle" onclick="viewDocumentDetails('${doc.id}')" title="View"><i class="bi bi-eye"></i></button>
        </td>`;
    return row;
}
...
window.changeMaterialsLimit = function() {
    ...
    if (limitDropdown) {
        ...
        window.loadMaterials(1, currentMaterialsStatus, currentMaterialsSearch, currentMaterialsDept);
    }
};
...
window.loadMaterials = async function(page = 1, status = undefined, search = undefined, dept = undefined) {
    ...
    try {
        const p = newSearchParams({ type: 'materials', page, limit: materialsPageSize, status: currentMaterialsStatus, search: currentMaterialsSearch, department: currentMaterialsDept });
        const d = await apiFetch(`${ADMIN_DATA_API}?${p}`);
        if (d.success) {
            publicMaterials = (d.materials || []).filter(m => {
                const okStatus = typeof matchesStatusFilter === 'function'
                    ? matchesStatusFilter(m.status, currentMaterialsStatus || 'all', 'document')
                    : true;
                const okDept = typeof matchesDepartmentFilter === 'function'
                    ? matchesDepartmentFilter(m.department || '', currentMaterialsDept || '')
                    : true;
                return okStatus && okDept;
            });
...
function createMaterialRow(m) {
    ...
    row.innerHTML = `
        ...
        <td class="text-end pe-4">
            <button class="btn btn-ghost btn-sm text-info rounded-circle" onclick="viewMaterialDetails('${m.id}')" title="View"><i class="bi bi-eye"></i></button>
        </td>`;
    return row;
}
